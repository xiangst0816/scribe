...
- Keep product facts consistent across languages: what Scribe does, supported platforms, install commands, model names, privacy behavior, repository layout, and architecture.
...
- Keep commands, file paths, class names, model identifiers, and code symbols unchanged.
...
- Simplified Chinese (`README.zh-Hans.md`): use natural Mainland Chinese technical writing. Prefer terms like "本地", "语音质量", "源码", "隐私说明", and avoid overly literal phrases such as "端侧运行" unless the context specifically needs them.
...
- Confirm model identifiers are exact:
  - `openai_whisper-base`
  - `openai_whisper-small_216MB`
  - `openai_whisper-large-v3-v20240930_626MB`
- Confirm privacy claims match the implementation: recognition is local after model download; model download uses Hugging Face; the legacy LLM refinement path is disabled by default and not shown in the menu.
