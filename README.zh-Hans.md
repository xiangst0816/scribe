...
- **可选本地润色**（默认关）：在「润色设置」里打开后，可以选择系统内置模型（macOS 26 + Apple Intelligence 可用区）或 Scribe 自带的 Qwen2.5-1.5B 本地模型（约 1 GB，按需下载）。两条路径都完全在本机推理，转写文本不会离开你的 Mac。
...
- 可选的转写润色（默认关）有两条引擎路径，**推理都在本机进行**：
  ...
  - *Scribe 本地模型* — Qwen2.5-1.5B-Instruct（约 1 GB）。第一次启用时从 ModelScope 或 HuggingFace 下载一次；下载 URL 和 SHA-256 校验值都写死在二进制里。下载完成后所有润色都完全离线运行。
...
│   └── LocalPolishService     ── 通过 llama.cpp 调 Qwen2.5-1.5B GGUF + 下载层
...
- [Qwen2.5-1.5B-Instruct](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF)（阿里云通义团队）：本地润色模型（Apache 2.0）。
