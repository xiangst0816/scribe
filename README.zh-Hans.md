...
它常驻在菜单栏。按住 **Fn** 键开始说，松开后，识别出的文字会自动粘贴到当前获得焦点的输入位置。Safari、VS Code、Slack、备忘录、网页输入框、终端，都可以直接用。如果松开 `Fn` 时当前没有可输入的焦点（比如停在桌面、播放按钮上），Scribe 不会"硬粘"，而是把文字放进剪贴板，并在浮条上短暂提示「已复制到剪贴板」，避免这次的语音被悄悄丢掉，你可以稍后自己 `⌘V` 粘到想要的地方。
...
- **剪贴板兜底**：松开 `Fn` 时如果没有可输入的焦点，转写文本会落到剪贴板上，浮条短暂显示「已复制到剪贴板」，避免这次的录音被默默吞掉。
...
| 松开 `Fn` | 经过约 500 毫秒尾部缓冲后停止录音，把文字粘贴到当前光标处。如果当前没有可输入的焦点，文字会改为放进剪贴板，浮条短暂提示「已复制到剪贴板」。 |
...
├── TextInjector           ── 剪贴板 + ⌘V 注入文本，并处理输入法切换；无可输入焦点时降级为只复制
├── FocusedFieldDetector   ── 基于 AX 检测当前是否有可输入焦点，用来决定走粘贴还是只复制
