...
- **屏幕上下文增强润色**（实验性，默认关）：润色开启后可以再勾一个子开关，按下 `Fn` 时对当前聚焦窗口截图并用 Apple Vision 文字识别，识别结果作为「用户正在看什么」的提示喂给润色模型，让屏幕上能看到的专有名词、文件名、标识符在润色时拼写更一致。需要屏幕录制权限。截图只在内存中处理，不落盘也不上传。
...
- **屏幕上下文增强润色**也是默认关的实验性子开关。打开后，按下 `Fn` 时 Scribe 会对当前聚焦窗口截一张图并在本机用 Apple Vision 做文字识别，把识别结果作为提示一并喂给润色模型，识别完成后图像即丢弃。需要屏幕录制权限。截图只在内存中处理，不会落盘、不会上传。每次抓取的内容都会写到 `~/Library/Logs/Scribe.log`，便于核对。
...
│   ├── ScreenContextCapture   ── （可选）屏幕上下文调度 + 日志写入
│   └── OCRContextSource       ── ScreenCaptureKit 截屏 + Vision OCR，把识别结果喂给润色
