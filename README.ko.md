...
- **화면 내용을 다듬기에 활용**(실험적, 기본 꺼짐): 다듬기를 켠 뒤 추가로 켤 수 있는 보조 토글입니다. `Fn`을 누르는 순간 포커스된 창의 스크린샷을 찍고 Apple Vision 텍스트 인식을 실행한 뒤, 인식된 텍스트를 "사용자가 지금 보고 있는 것"의 힌트로 다듬기 모델에 전달합니다. 화면에 보이는 고유 명사, 파일 이름, 식별자가 다듬어진 결과에서도 일관되게 표기됩니다. 화면 기록 권한이 필요합니다. 스크린샷은 메모리에서만 처리되며 디스크에 저장되지 않고 외부로 전송되지도 않습니다.
...
- **화면 내용을 다듬기에 활용**도 기본 꺼짐인 실험적 보조 토글입니다. 켜면 `Fn`을 누르는 순간 Scribe가 포커스된 창의 스크린샷을 한 장 찍고 Mac에서 Apple Vision으로 텍스트 인식을 실행하며, 인식 결과는 다듬기 모델의 힌트로 전달되고 이미지는 인식 후 즉시 폐기됩니다. 화면 기록 권한이 필요합니다. 이미지는 메모리에서만 처리되며 디스크에 기록되지도, 외부로 전송되지도 않습니다. 각 캡처 활동은 `~/Library/Logs/Scribe.log`에 기록되어 확인할 수 있습니다.
...
│   ├── ScreenContextCapture   ── (선택) 화면 컨텍스트 디스패처 + 로그 싱크
│   └── OCRContextSource       ── ScreenCaptureKit 캡처 + Vision OCR로 다듬기 모델에 전달
