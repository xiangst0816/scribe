...
把现在那块 600 字符的固定 system prompt 拆成三段，**运行时拼装**：

```
┌──────────────────────────────────────────────┐
│  Layer A — 固定指令（in code，不可变）        │
│   • 角色（你是 transcript polisher）         │
│   • 必须 / 必须不做的硬规则                  │
│   • 输出格式约束（"only the polished text"）│
│   • language_hint 占位符                     │
└──────────────────────────────────────────────┘
                       ▼
┌──────────────────────────────────────────────┐
│  Layer B — 动态/通用上下文（参数化）         │
│   • 当前 dictation locale → language_hint    │
│   • 当前前台 app（可选，未来扩展）            │
│   • 内置 few-shot 示例（per-language）        │
└──────────────────────────────────────────────┘
                       ▼
┌──────────────────────────────────────────────┐
│  Layer C — 用户口吻档案（学习得到，可重置）  │
│   • "casual register, contractions OK"        │
│   • "frequent technical terms: Swift, iOS"   │
│   • "code-switches en/zh inline"              │
│   • 3-8 行精简描述，硬上限 ~200 token        │
└──────────────────────────────────────────────┘
```
...
Personalization (learned from your recent dictations):
[Layer C 文本，若空则整段省略]
...
| 层 | 存放位置 | 谁能修改 |
| --- | --- | --- |
| A 固定指令 | Swift 源码（`PolishPrompt.kCorePrompt`） | 只有 Scribe 开发者（随版本升级） |
| B 动态上下文 | Swift 源码 + locale 参数 | 运行时按 `selectedLocaleCode` 选 |
| C 用户档案 | `~/Library/Application Support/Scribe/style/profile.txt` | 用户可见、可编辑、可重置 |

A 和 B 在内存里组装；C 是磁盘文件。Scribe 启动时一次性读 C，缓存在 `PolishPrompt.userProfile`，每次 polish 拼到 prompt 末尾。
...
**只看 raw 转写**，**不看 polished 输出**——polished 已经被 Qwen 改写过了，对学口吻是污染。

raw 转写的来源：每次 `AppleSpeechSession` 拿到 final transcript，在 `AppDelegate.deliverFinal` 那一步顺便写进一个**滚动缓冲**：

```
~/Library/Application Support/Scribe/style/transcript-history.jsonl
```

JSON Lines 格式，每行一条：

```json
{"ts": "2026-04-27T16:30:00Z", "lang": "zh-CN", "text": "嗯那个我觉得..."}
```

**滚动策略**：硬上限 50 条 / 14 天，到了删最旧的。文件大小预估 < 500 KB。
...
> 这是这个 feature 能不能上线的真正的硬门槛。Scribe 当前 README 明文写「audio is discarded after each push-to-talk」。引入 transcript history 必须明确划界。

### 4.1 必备开关

Settings 里在 Polish 主开关下面加一个独立 sub-section：

```
☐ Adapt to my voice over time
   Scribe will record your dictation transcripts (text only, no audio)
   to ~/Library/Application Support/Scribe/style/ to learn the words
   and tone you prefer. Off by default.

   Stored: 0 transcripts (max 50 / 14 days, rolling)
   [ View my voice profile ]   [ Reset ]   [ Open transcript folder ]
```

**默认关**。除非用户主动打勾，**任何转写都不会被持久化**。
...
> the *text* of your last 50 dictation transcripts (no audio) under
> `~/Library/Application Support/Scribe/style/` to derive a short
> profile of your vocabulary and tone. Everything stays on your Mac;
> nothing is uploaded. The folder + profile are visible and resettable
> from Settings.

### 4.3 可观测

- "View my voice profile" 按钮：直接打开 `profile.txt` 让用户看 Scribe 学到了啥。
- "Reset" 按钮：清空 history + profile。
- "Open transcript folder" 按钮：在 Finder 里展示存了什么。
