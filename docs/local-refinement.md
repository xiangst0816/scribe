...
| **Local (Scribe model)** | llama.cpp | Gemma 4 E2B-it Q4_K_M（~3.46 GB，bartowski 量化） | 用户首次启用时下载到 `~/Library/Application Support/Scribe/models/` | 任何 Apple Silicon Mac（不限 macOS 版本、不限区域） |
...
| Gemma 4 E2B-it Q4_K_M GGUF | Local backend 模型权重，**运行时下载，不打包进 .app** | Apache 2.0 |

### 2.3 为什么 Local backend 选 Gemma 4 E2B（v0.4.x 起）

**当前选择**：Gemma 4 E2B-it Q4_K_M（~3.46 GB，~2B active params / ~5B total via MatFormer，Apache 2.0）。

完整候选对比与决策依据见 [polish-model-eval.md](polish-model-eval.md)。**简版**：v0.3.x 用的 Qwen2.5-1.5B 在 36-case 探针上失败 ~12-14 项，包括 persona-leak（"我是向松涛"+"你知道我是谁吗"→"我知道你是向松涛"）、答非所问（"嗯你在干嘛"→AI 回答它在干嘛）、JA→ZH 翻译漂移等核心 bug。Gemma 4 E2B 同一探针通过 33/36，三大 headline bug 全部修复。代价：下载体积 1 GB → 3.46 GB。

历史否决候选（保留作背景）：
- **Qwen2.5-0.5B / Qwen3-0.6B**：体积小（~400 MB），但指令遵循质量明显跌一档 → 否决。
- **Llama-3.2-1B / 3B**：英语优秀，中文一般 → 否决。
- **Gemma 3 1B**：~700 MB 是诱人选项，但和 Qwen 1.5B 同档，没有突破。
- **Gemma 4 E4B**：~5.3 GB / ~4B active，延迟逼近 3s 上限，下载体积 5× 当前 → 否决（Round 2 储备）。
- **MoE 类（Mixtral / Qwen3-MoE）**：激活量大或下载巨大 → 否决。
- **Qwen2.5-3B**：3B Q4 ~1.9 GB，质量更高，但相对 E2B 优势不明显且没有 MatFormer 的快推理 → 备选不优先。

### 2.4 为什么 Local 引擎选 llama.cpp，不选 MLX-Swift

MLX 体验更原生，但社区 GGUF 不能直接喂——要重新转 MLX 权重并量化，工作量大且量化质量需要单独验证。llama.cpp 直接吃官方 GGUF，最快可用，且 Gemma 4 在 llama.cpp 主线已支持（用 `<|turn>...<turn|>` 模板）。MLX 留作后续优化。
...
│                                                            │
│    ○ Scribe local model (Gemma 4 E2B, downloads ~3.5 GB)   │
...
1. **确认对话框**——告知模型大小（~3.5 GB）、来源（当前镜像 URL）、磁盘占用位置（`~/Library/Application Support/Scribe/models/`）、是否继续。  
...
| Ready (Local) | 走 Gemma 4 E2B。**显式带 "Local" 字样**，让用户随时清楚当前是哪条路径 | 跳到 Settings |
...
~/Library/Application Support/Scribe/
  └── models/
      ├── gemma-4-E2B-it-Q4_K_M.gguf                   (~3.46 GB，最终文件)
      ├── gemma-4-E2B-it-Q4_K_M.gguf.partial           (下载中临时文件)
      └── gemma-4-E2B-it-Q4_K_M.gguf.partial.meta      ({ url, expected_size, expected_sha256, started_at, completed_bytes })
```
...
- 当前 release zip ~10 MB，加上 ~3.5 GB 模型会让 Sparkle delta 升级失效（每次小升级都重传几 GB）。
...
| 项 | System (Apple FM) | Local (Gemma 4 E2B) |
| --- | --- | --- |
| 模板载体 | `LanguageModelSession(instructions: String?)` | llama.cpp + Gemma 4 turn 格式 |
...
**Gemma 4 turn 模板**（与 Gemma 3 的 `<start_of_turn>` 不同；管道位置不对称是 Google 故意设计的；BOS 由 `llama_tokenize(add_special: true)` 自动加；**不**注入 `<|think|>` token，E2B 在 thinking off 时不产生 thought 块）：

```
<|turn>system
{{system_prompt}}<turn|>
<|turn>user
Polish the dictation between <RAW> markers. ...
<RAW>
{{raw_transcript}}
</RAW>
<turn|>
...
stop tokens: `<turn|>`、EOS（llama.cpp 已把 `<turn|>` 列入 EOG token set）。

输出剥离层防御性删除：`<|turn>` / `<turn|>` / `<|channel>` / `<channel|>` 加上历史 Qwen ChatML 标记和泛用 `<bos>` / `<eos>`，并把 `<|channel>thought ... <channel|>` 整段擦除——E2B with thinking off 文档说不产生这些，但万一某次 build 回退写一行 guard 防止思考文本泄漏到光标。
...
1. **五份 README** "Privacy" 章节加一段：
   ...
   > - **Scribe local model** — downloads Gemma 4 E2B-it (~3.5 GB) once from HuggingFace or ModelScope to `~/Library/Application Support/Scribe/`. After download, all polishing runs entirely locally. Both the download URL and SHA-256 are baked into the binary; no other network traffic is involved.
...
4. **Acknowledgements**（README + 应用菜单）：补 llama.cpp（MIT）和 Gemma 4 E2B-it（Apache 2.0）的归属与许可。
...
3. **首次下载体验**：~3.5 GB 在国内非校园网平均 ~15-20 分钟（v0.4.x 起，从原本 1 GB 涨上来；做权衡见 [polish-model-eval.md](polish-model-eval.md)）。决定下载窗口是常驻 Settings 进度条 + 状态栏小角标，还是浮窗——倾向前者。
4. **Sparkle delta 升级**：模型不打包进 .app，所以 Sparkle 升级不会触发模型重下；但模型升级（v0.3.x Qwen2.5 → v0.4.x Gemma 4 E2B；未来可能再升级）需要单独的版本号字段（写在 `.partial.meta` 同级的 `model.version` 里），由 app 启动时比对决定要不要重下。
5. **小模型输出污染**：偶发输出 "Sure, here's the polished text:" 之类前言、Gemma 4 turn token、或 thinking-block 残骸。处理顺序：先用 prompt 堵住（已在 §5.3 中"Output ONLY the polished text" + 不注入 `<|think|>`），实测仍有则做轻量 strip——**只剥常见模式**，绝不要正则把真内容也吃掉。
...
8. **文档同步**：五份 README 和 CLAUDE.md（参考 CLAUDE.md 的 pre-push checklist）；Acknowledgements 加 llama.cpp + Gemma 4 E2B-it。

---

## 10. 不在范围内（避免设计扩散）

- 多模型选择 UI：永远只一个 SKU（当前为 Gemma 4 E2B-it Q4_K_M；切换 SKU 是开发侧的 product decision，不暴露给用户）。
- 自定义 prompt：不暴露给用户改。
- Streaming 输出：润色是一次性的，不需要流式。
- 分布式下载 / P2P：~3.5 GB 单连接续传足矣（HTTP Range + 镜像 fallback 已覆盖）。
