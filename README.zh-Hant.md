...
它會待在選單列。按住 **Fn** 開始說話，放開後，辨識出的文字會自動貼到目前取得焦點的輸入位置。Safari、VS Code、Slack、備忘錄、網頁輸入框、終端機，都可以直接用。如果放開 `Fn` 時剛好沒有可輸入的焦點（停在桌面、停在某個播放按鈕上之類），Scribe 不會硬貼，而是把文字放進剪貼板，浮條上短暫顯示「已複製到剪貼板」，避免這次的語音悄悄消失，你可以等一下自己 `⌘V` 貼到想要的位置。
...
- **剪貼板備援**：放開 `Fn` 時若沒有可輸入的焦點，轉寫文字會落到剪貼板，浮條短暫顯示「已複製到剪貼板」，避免這次的錄音被默默吞掉。
...
| 放開 `Fn` | 約 500 毫秒尾端緩衝後結束錄音，把文字貼到目前游標處。如果當下沒有可輸入的焦點，文字會改為放進剪貼板，浮條短暫顯示「已複製到剪貼板」。 |
...
├── TextInjector           ── 剪貼簿 + ⌘V 注入文字，並處理輸入法切換；無可輸入焦點時改為只複製
├── FocusedFieldDetector   ── 基於 AX 偵測目前是否有可輸入焦點，決定走貼上還是只複製
