...
- **可選本機潤飾**（預設關閉）：在「潤飾設定」中可開啟，並選擇系統內建模型（macOS 26 + Apple Intelligence 可用區）或 Scribe 自帶的 Gemma 4 E2B 本機模型（約 3.5 GB，需下載）。兩條路徑的推理皆完全在本機進行，轉寫文字不會離開你的 Mac。
- **以螢幕內容輔助潤飾**（實驗性，預設關閉）：開啟潤飾後可再勾一個子開關，按下 `Fn` 時對目前聚焦視窗截圖並用 Apple Vision 進行文字辨識，辨識結果作為「使用者正在看什麼」的提示傳入潤飾模型，讓螢幕上看到的專有名詞、檔名、識別碼在潤飾時拼寫更一致。需要螢幕錄製權限。截圖只在記憶體中處理，不寫入磁碟也不會上傳。
...
| `~/Library/Preferences/com.yetone.Scribe.plist` | UserDefaults（已選語言、潤飾開關） |
| `~/Library/Application Support/Scribe/models/` | 本機潤飾模型（僅在啟用 Local 引擎並完成下載後才會出現） |
...
- 可選的轉寫潤飾（預設關閉）有兩條引擎，**推理皆在本機進行**：
  - *系統內建* — 使用 macOS 自帶的 Apple Intelligence 端側模型，無需下載，僅在 macOS 26+ 且當前地區支援時可用。
  - *Scribe 本機模型* — Gemma 4 E2B-it（約 3.5 GB）。第一次啟用時從 ModelScope 或 HuggingFace 下載一次；下載 URL 與 SHA-256 校驗值都寫死在二進位中。下載完成後所有潤飾完全離線執行。
- 啟用潤飾後，轉寫文字會先經過所選引擎再貼上；任何超時或錯誤都會立即降級為原文，不會讓錄音遺失。
- **以螢幕內容輔助潤飾**也是預設關閉的實驗性子開關。開啟後，按下 `Fn` 時 Scribe 會對目前聚焦視窗截一張圖並在本機用 Apple Vision 做文字辨識，將辨識結果作為提示一併傳入潤飾模型，辨識完成後影像即釋放。需要螢幕錄製權限。截圖只在記憶體中處理，不會寫入磁碟、不會上傳。每次擷取的內容都會寫到 `~/Library/Logs/Scribe.log` 以便核對。
...
├── AppleSpeechSession     ── 基於 SFSpeechRecognizer 的串流辨識，附帶音量計
├── OverlayPanel           ── 無邊框 NSPanel，提供波形膠囊和上方的即時字幕浮條
├── TextInjector           ── 剪貼簿 + ⌘V 注入文字，並處理輸入法切換
├── Refinement/            ── 可選的轉寫潤飾（預設關閉）
│   ├── PolishCoordinator      ── 引擎仲裁、3 秒超時、連續失敗熔斷
│   ├── SystemPolishService    ── Apple Intelligence（macOS 26+，受地區限制）
│   ├── LocalPolishService     ── 透過 llama.cpp 呼叫 Gemma 4 E2B GGUF + 下載層
│   ├── ScreenContextCapture   ── （可選）螢幕上下文派發 + 日誌寫入
│   └── OCRContextSource       ── ScreenCaptureKit 截圖 + Vision OCR，把辨識結果餵給潤飾
├── SettingsWindow         ── 潤飾總開關 + 系統/本機引擎二選一
└── AppDelegate            ── 選單列 UI、狀態圖示和錄音生命週期
```

App 程式碼分成 `ScribeCore` 函式庫和一個極簡可執行入口。專案沒有 Xcode project，只有 [Package.swift](Package.swift) 和一個小的 [Makefile](Makefile)，用來串起 `swift build`、`.app` 打包和 ad-hoc 簽名。執行測試用 `swift test`（XCTest 需要 Xcode；CI 使用 `macos-15` runner）。

llama.cpp 透過 SwiftPM 的 `binaryTarget` 引入官方發行的 `xcframework`，本機建置不需要 CMake 或 Xcode。連結到 .app 後僅增加約 9 MB；模型權重單獨存放於 `~/Library/Application Support/Scribe/`。

## 致謝

- [Sparkle](https://sparkle-project.org)：自動更新框架。
- Apple [Speech](https://developer.apple.com/documentation/speech) 框架：底層辨識能力。
- [llama.cpp](https://github.com/ggml-org/llama.cpp)：本機模型推理引擎（MIT）。
- [Gemma 4 E2B-it](https://huggingface.co/bartowski/google_gemma-4-E2B-it-GGUF)（Google，GGUF 量化由 bartowski 提供）：本機潤飾模型（Apache 2.0）。
