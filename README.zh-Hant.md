...
- **可選本機潤飾**（預設關閉）：在「潤飾設定」中可開啟，並選擇系統內建模型（macOS 26 + Apple Intelligence 可用區）或 Scribe 自帶的 Qwen2.5-1.5B 本機模型（約 1 GB，需下載）。兩條路徑的推理皆完全在本機進行，轉寫文字不會離開你的 Mac。
...
- 可選的轉寫潤飾（預設關閉）有兩條引擎，**推理皆在本機進行**：
  ...
  - *Scribe 本機模型* — Qwen2.5-1.5B-Instruct（約 1 GB）。第一次啟用時從 ModelScope 或 HuggingFace 下載一次；下載 URL 與 SHA-256 校驗值都寫死在二進位中。下載完成後所有潤飾完全離線執行。
...
│   └── LocalPolishService     ── 透過 llama.cpp 呼叫 Qwen2.5-1.5B GGUF + 下載層
...
- [Qwen2.5-1.5B-Instruct](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF)（阿里雲通義團隊）：本機潤飾模型（Apache 2.0）。
