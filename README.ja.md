...
Scribe はメニューバーに常駐します。**Fn** を押しながら話し、離すと、認識されたテキストが現在フォーカスされている入力欄に自動で貼り付けられます。Safari、VS Code、Slack、メモ、Web の入力欄、ターミナルなどでそのまま使えます。`Fn` を離した瞬間にフォーカスされている入力欄がない場合（デスクトップや再生ボタンなどに当たっているとき）、Scribe は無理に貼り付けようとせず、文字起こしをクリップボードに退避させ、ピルに「クリップボードにコピーしました」と短く表示します。あとから自分で `⌘V` で好きな場所に貼り付けてください。
...
- **クリップボードへのフォールバック**：`Fn` を離したときに編集可能な入力欄にフォーカスがない場合、文字起こしをクリップボードに置き、ピルに短く「クリップボードにコピーしました」と表示します。録音が音もなく失われることはありません。
...
| `Fn` を離す | 約 500 ミリ秒の末尾バッファののち録音を終了し、現在のカーソル位置にテキストを貼り付けます。編集可能な入力欄にフォーカスがない場合は、代わりにクリップボードへ退避させ、ピルに「クリップボードにコピーしました」と短く表示します。 |
...
├── TextInjector           ── クリップボード + ⌘V でテキストを挿入し、入力ソース切替も処理。入力欄にフォーカスがない場合は「コピーのみ」にフォールバック
├── FocusedFieldDetector   ── AX で編集可能な入力欄のフォーカス有無を判定し、貼り付けるか「コピーのみ」にするかを決める
