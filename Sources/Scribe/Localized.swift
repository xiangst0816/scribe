...
enum L10n {
    ...
    private static let strings: [String: [String: String]] = [
        "en": [
            ...
            "settings.polish.screenContext.label":  "Use screen context for polish (experimental)",
            "settings.polish.screenContext.detail": "When you press Fn, Scribe captures a screenshot of the focused window and runs Vision OCR on it, feeding the recognized text to polish so proper nouns, file names, and identifiers visible on screen are spelled consistently. Requires Screen Recording permission. See ~/Library/Logs/Scribe.log for what was captured each time.",
        ...
        "zh-Hans": [
            ...
            "settings.polish.screenContext.label":  "把屏幕内容作为润色上下文（实验性）",
            "settings.polish.screenContext.detail": "按下 Fn 时对当前聚焦窗口截图并用 Vision OCR 识别文字，作为润色模型的背景参考，让屏幕上能看到的专有名词、文件名、标识符在润色时拼写更一致。需要屏幕录制权限。每次抓到了什么都会写到 ~/Library/Logs/Scribe.log。",
        ...
        "zh-Hant": [
            ...
            "settings.polish.screenContext.label":  "把螢幕內容作為潤飾上下文（實驗性）",
            "settings.polish.screenContext.detail": "按下 Fn 時對目前聚焦視窗截圖，再用 Vision OCR 辨識文字作為潤飾模型的背景參考，讓螢幕上看到的專有名詞、檔名、識別碼在潤飾時拼寫更一致。需要螢幕錄製權限。每次擷取到的內容都會寫進 ~/Library/Logs/Scribe.log。",
        ...
        "ja": [
            ...
            "settings.polish.screenContext.label":  "画面の内容を整形のコンテキストに利用する（実験的）",
            "settings.polish.screenContext.detail": "Fn を押した瞬間にフォーカス中のウィンドウをスクリーンショットして Vision OCR で文字認識し、その結果を整形モデルに背景情報として渡します。画面に映っている固有名詞、ファイル名、識別子が整形後のテキストでも一貫して綴られるようになります。画面収録の権限が必要です。毎回取得された内容は ~/Library/Logs/Scribe.log で確認できます。",
        ...
        "ko": [
            ...
            "settings.polish.screenContext.label":  "화면 내용을 다듬기 컨텍스트로 사용 (실험적)",
            "settings.polish.screenContext.detail": "Fn을 누른 순간 포커스된 창의 스크린샷을 찍고 Vision OCR로 텍스트를 인식한 뒤, 다듬기 모델에 배경 정보로 전달합니다. 화면에 보이는 고유 명사, 파일 이름, 식별자가 다듬어진 결과에서도 일관되게 표기됩니다. 화면 기록 권한이 필요합니다. 매번 캡처된 내용은 ~/Library/Logs/Scribe.log에서 확인할 수 있습니다.",
