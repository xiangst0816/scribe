...
final class SystemPolishService: PolishService {
    ...
    /// Stable digest of the assembled system prompt. `nonisolated` because
    /// it's a pure function — no actor-local state — and the tests call it
    /// from a non-MainActor `@Suite struct`.
    nonisolated static func promptCacheKey(_ systemPrompt: String) -> String {
